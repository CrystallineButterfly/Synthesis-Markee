...
import {RepoStewardCampaign} from "src/RepoStewardCampaign.sol";

contract RepoStewardCampaignTest is Test {
    RepoStewardCampaign private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_markee");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new RepoStewardCampaign(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        RepoStewardCampaign freshController = new RepoStewardCampaign(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
