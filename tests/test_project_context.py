...
MODULE = importlib.import_module('agents.repo_steward')
...
class ProjectContextTest(unittest.TestCase):
    ...
    def test_primary_names_match(self) -> None:
        spec = MODULE.build_project_spec()
        self.assertEqual(spec.primary_python_module, 'repo_steward')
        self.assertEqual(spec.primary_contract_name, 'RepoStewardCampaign')
